...
let btnOne = document.querySelector(".btn-sort-name");
let btnTwo = document.querySelector(".btn-sort-capital");
let btnThree = document.querySelector(".btn-sort-population");
function presentCountries(usingMap) {
  console.log(usingMap);
  boxesContainer.innerHTML = "";
  usingMap.map(function (country) {
    boxesContainer.innerHTML += `<div class="box"><div>${
      country.name
    }</div><div>${country.capital}</div><div>${
      country.languages
    }</div><div>${country.population.toLocaleString()}</div></div>`;
  });
}
presentCountries(countriesObject);
...
function filterd(userInput) {
  let filterCountris = countriesObject.filter(function (country) {
    return country.name.toLowerCase().startsWith(userInput);
  });
  presentCountries(filterCountris);
}

function AzReverese(arr) {
  // spread opeator //
  ...
  if (nameAZ) {
    nameAZ = false;
    // console.log(a.name);
    sortRByName = copyCountriesObject.sort((a, b) =>
      a.name < b.name ? 1 : -1
    );
  } else {
    nameAZ = true;
  ...
  console.log(sortRByName);
  presentCountries(sortRByName);
}

function sorByPopulation(arr) {
  // spread opeator //
  const copyCountriesObject = [...arr];
  let sortPopulation;
  if (population) {
    population = false;
    sortPopulation = copyCountriesObject.sort((a, b) =>
  ...
  presentCountries(sortPopulation);
}
function sortByCapital(arr) {
  ...
  if (capitalAZ) {
    capitalAZ = false;
    sortCapital = copyCountriesObject.sort(function (a, b) {
    ...
    capitalAZ = true;
    sortCapital = copyCountriesObject.sort((a, b) =>
      a.capital < b.capital ? -1 : 1
    );
  }
  presentCountries(sortCapital);
}

btnOne.addEventListener("click", function () {
  AzReverese(countriesObject);
});
...
btnThree.addEventListener("click", function () {
  sorByPopulation(countriesObject);
});
